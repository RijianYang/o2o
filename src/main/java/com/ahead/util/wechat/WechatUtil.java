...
public class WechatUtil {
    ...
    public static User wechatUserToUser(WechatUser wechatUser) {
        User user = new User();
    ...
    public static UserAccessToken getUserAccessToken(String code) throws IOException {
        // 测试号信息里的appId
        String appId = "wx7626c44e2aca26fd";
        log.debug("appId:" + appId);
        // 测试号信息里的appsecret
        String appsecret = "5c0d7b6f80512caff4e7bfcc226a7f82";
        log.debug("secret:" + appsecret);
        // 根据传入的code,拼接出访问微信定义好的接口的URL
        String url = "https://api.weixin.qq.com/sns/oauth2/access_token?appid=" + appId + "&secret=" + appsecret
                + "&code=" + code + "&grant_type=authorization_code";

        //微信规定Code只能使用一次，但这里会并发访问多次，所以将tokenStr放入session中，每次都从session中拿
        HttpSession session = RequestUtil.getRequest().getSession();
        String sessionTokenStr = null;
        synchronized (session) {
            sessionTokenStr = (String) session.getAttribute("tokenStr");
            if (sessionTokenStr == null || "".equals(sessionTokenStr) || sessionTokenStr.contains("errmsg")) {
                //如果session中没有请求到微信获取
                // 向相应URL发送请求获取token json字符串
                sessionTokenStr = httpsRequest(url, "GET", null);
                session.setAttribute("tokenStr", sessionTokenStr);
            }

            System.out.println(sessionTokenStr);
        }

        log.debug("userAccessToken:" + sessionTokenStr);
        UserAccessToken token = new UserAccessToken();
        ObjectMapper objectMapper = new ObjectMapper();
        try {
            // 将json字符串转换成相应对象
            token = objectMapper.readValue(sessionTokenStr, UserAccessToken.class);
